...
# Packages ----
pkgs <- c("Hmisc", "tidyverse", "LSAfun", "broom", "ngram")
xfun::pkg_attach(pkgs)

# Source ----
# Requires you run these scripts prior for most updated data:
source("dod-prepro.R")
...
load("EN_100k.rda")                 # Semantic space for LSA
# SUBTLEX
...
save(lsa_data, file = "lsa-data.RData") # Saves out LSA data
...
wf_data <- bind_rows(ctl_hsct_calc_subtlus_ss,
                     dod_hsct_tbi_subtlus_ss,
                     dod_hsct_ctl_subtlus_ss
                     )

save(wf_data, file = "wf-data.RData") # Saves out WF and CD data

# Clean up variables ----
rm(control_hsct_data,
   ctl_hsct_calc,
