...
# 
write_csv(demo_neuropsych_clean %>% select(DODID, group), path = "final-ss.csv")
...
This is looking at a targeted approach to differentiate mild vs. controls and mod/severe vs. controls:

```{r}
## MILDS
demo_neuropsych_clean_cvm <- 
  demo_neuropsych_clean %>% 
  filter(osuWorstInjury %in% c("Control", "Mild")) %>%
  mutate(osuWorstInjury = factor(osuWorstInjury)) # resets factor for contrast

contrasts(demo_neuropsych_clean_cvm$osuWorstInjury) <- cbind(c_v_m = c(.5, -.5))

# Models
cat_A_mod_cvm <- 
  lm(
    cat.a.errors ~ 1 + 
    scale(age, scale = FALSE) * osuWorstInjury + 
    scale(wasi.full2IQ, scale = FALSE) * osuWorstInjury, 
  data = demo_neuropsych_clean_cvm
  )
summary(cat_A_mod_cvm) # to view model
...
summary(cat_B_mod_cvm) # to view model
...
summary(hsct_mod_cvm) # to view model

# Summary for plots
demo_cvm_sum <- 
  demo_neuropsych_clean_cvm %>% 
  select(DODID, osuWorstInjury, age, wasi.full2IQ, cat.a.errors, cat.b.errors, hsct.scaled) %>% 
  gather(meas, value, -DODID, -osuWorstInjury) %>%
  group_by(osuWorstInjury, meas) %>%
...
# Category A errors
ggplot(
  demo_cvm_sum %>% filter(meas == "cat.a.errors"), 
  aes(osuWorstInjury, m)
  ) +
  geom_point(
    data = demo_neuropsych_clean_cvm, 
    aes(y = cat.a.errors),
  ...
  labs(x = "TBI Severity", y = "Category A Errors") +
  theme_minimal()

# Category B Errors
ggplot(
  demo_cvm_sum %>% filter(meas == "cat.b.errors"), 
  aes(osuWorstInjury, m)
  ) +
  geom_point(
    data = demo_neuropsych_clean_cvm, 
  ...
  geom_point(size = 3, shape = 17) +
  geom_errorbar(aes(ymin = m - sem, ymax = m + sem), width = .1) +
  labs(x = "TBI Severity", y = "Category B Errors") +
  theme_minimal()

# Category B Errors
ggplot(
  demo_cvm_sum %>% filter(meas == "hsct.scaled"), 
  aes(osuWorstInjury, m)
  ) +
  geom_point(
    data = demo_neuropsych_clean_cvm, 
  ...
  geom_point(size = 3, shape = 17) +
  geom_errorbar(aes(ymin = m - sem, ymax = m + sem), width = .1) + 
  labs(x = "TBI Severity", y = "HSCT Scaled Scores") +
  theme_minimal()

## MOD/SEVERE

demo_neuropsych_clean_cvs <- 
  demo_neuropsych_clean %>% 
  filter(osuWorstInjury %in% c("Control", "Mod/Severe")) %>%
  mutate(osuWorstInjury = factor(osuWorstInjury)) # resets factor for contrast

contrasts(demo_neuropsych_clean_cvs$osuWorstInjury) <- cbind(c_v_s = c(.5, -.5))
...
cat_A_mod_cvs <- 
  lm(
    ...
    scale(age, scale = FALSE) * osuWorstInjury + 
    scale(wasi.full2IQ, scale = FALSE) * osuWorstInjury,
  data = demo_neuropsych_clean_cvs
  )
summary(cat_A_mod_cvs) # to view model
...
summary(cat_B_mod_cvs) # to view model
...
summary(hsct_mod_cvs) # to view model
...
# Category A errors
ggplot(
  demo_cvs_sum %>% filter(meas == "cat.a.errors"), 
  aes(osuWorstInjury, m)
  ) +
  geom_point(
    data = demo_neuropsych_clean_cvs, 
...
# Category B Errors
ggplot(
  demo_cvs_sum %>% filter(meas == "cat.b.errors"), 
  aes(osuWorstInjury, m)
  ) +
  geom_point(
    data = demo_neuropsych_clean_cvs, 
...
# HSCT scaled scores
ggplot(
  demo_cvs_sum %>% filter(meas == "hsct.scaled"), 
  aes(osuWorstInjury, m)
  ) +
  geom_point(
    data = demo_neuropsych_clean_cvs, 
...
ggsave("paper-figure-lsa-sd-error-bars.svg", width = 95, height = 117, units = "mm")
...
ggsave("paper-figure-lsa-sem-error-bars.svg", width = 95, height = 117, units = "mm")
...
ggsave("paper-figure-hsct-time-sem-error-bars.svg", width = 95, height = 117, units = "mm")
...
ggsave("paper-figure-hsct-time-sd-error-bars.svg", width = 95, height = 117, units = "mm")
```
...
hayling_desc_table <- bind_rows(c1_desc, c2_desc, c3_desc) %>%
...
# writes out to csv
write_csv(hayling_desc_table, path = "paper-table-hsct-desc.csv")
```

## Control vs. TBI Groups

```{r}
# Combines the data
rep_data <- 
  left_join(
    lsa_sum_iq, wf_sum_iq %>% select(DODID, wf_diff, cd_diff), 
    by = "DODID"
    )
 
# Separates controls and mild
rep_data_cvm <- 
  rep_data %>% 
  filter(osuWorstInjury %in% c("Control", "Mild")) %>%
  mutate(osuWorstInjury = factor(osuWorstInjury)) # resets factor

# Separates controls and mod/severe
rep_data_cvs <- 
  rep_data %>% 
  filter(osuWorstInjury %in% c("Control", "Mod/Severe")) %>%
  mutate(osuWorstInjury = factor(osuWorstInjury)) # resets factor
...
# Sets contrasts
contrasts(rep_data_cvm$osuWorstInjury) <- cbind(c_v_m = c(.5, -.5))
contrasts(rep_data_cvs$osuWorstInjury) <- cbind(c_v_s = c(.5, -.5))
...
Visualizing interesting effects from above

```{r}
# MILD VS. CONTROLS: Age x Group interaction for RT (p = .0157)
ggplot(
  rep_data_cvm, 
  aes(age, uc.time.raw - sc.time.raw, group = osuWorstInjury, color = osuWorstInjury)
  ) +
  geom_point() +
  geom_smooth(method = "lm") +
  labs(x = "Age (years)", y = "RT (UC - SC)") +
  theme_minimal() +
  theme(legend.position = "bottom")

# MILD VS. CONTROLS: Age x Group interaction for lsa (p = .098)
ggplot(
  rep_data_cvm, 
  aes(age, lsa_diff, group = osuWorstInjury, color = osuWorstInjury)
  ) +
  geom_point() +
  geom_smooth(method = "lm") +
  labs(x = "Age (years)", y = "LSA (SC - UC)") +
  theme_minimal() +
  theme(legend.position = "bottom")

# MOD/SEVERE VS. CONTROLS: IQ x Group cateogry B errors(p = .068)
ggplot(
  rep_data_cvs, 
  aes(wasi.full2IQ, cat.b.errors, group = osuWorstInjury, color = osuWorstInjury)
  ) +
  geom_point(position = "jitter") +
  geom_smooth(method = "lm") +
  labs(x = "WASI Full 2 IQ", y = "Category B Errors") +
  theme_minimal() +
  theme(legend.position = "bottom")

# MOD/SEVERE VS. CONTROLS: IQ x Group HSCT scaled scores (p = .07)
ggplot(
  rep_data_cvs, 
  aes(wasi.full2IQ, hsct.scaled, group = osuWorstInjury, color = osuWorstInjury)
  ) +
  geom_point(position = "jitter") +
  geom_smooth(method = "lm") +
  labs(x = "WASI Full 2 IQ", y = "HSCT Scaled Scores") +
  theme_minimal() +
  theme(legend.position = "bottom")

# MOD/SEVERE VS. CONTROLS: Group effect on time (p = .0075)
time_cvs_ss <- 
  rep_data_cvs %>% 
  select(DODID, osuWorstInjury, sc.time.raw, uc.time.raw) %>%
  gather(meas, value, -DODID, -osuWorstInjury)
...
ggplot(
  time_cvs_group, 
  aes(meas, m, group = osuWorstInjury, color = osuWorstInjury)
  ) +
  geom_point(data = time_cvs_ss, aes(meas, value), shape = 1, position = pj, alpha = 1/2) +
  geom_point(position = pd) +
  geom_errorbar(aes(ymin = m - sem, ymax = m + sem), width = .1, position = pd) + 
  geom_path(position = pd) +
  theme_minimal()
...
## Number of responses not found in either corpus
...
# Number of responses not found by subtlex
sum(wf_na_data$total_na)

# Number of responses not found with LSA
